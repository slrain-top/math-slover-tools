...
def ReadComment(reader):
    ...
    while (reader.LookingAt(" *//")
           or reader.LookingAt("#ifndef PROTO2_OPENSOURCE")
           or reader.LookingAt("#endif  // !PROTO2_OPENSOURCE")):
        if reader.LookingAt(" *//"):
...
def HandleComment(reader, out, group_tracker):
    ...
    if ((comment[0].count("----") > 0 or comment[0].count("====") > 0)
            and comment[0].startswith("  ")):
        ...
        out.offset += 1
            ...
            not reader.LookingAt(" *class +\\w+ *;")
            and not reader.LookingAt(" *namespace +\\w+ *{")
            and not reader.LookingAt(" *#") and
...
def FindNextComment(reader, out, group_tracker):
    while not reader.LookingAt(" *//") and not reader.Eof():
        ...
        if (reader.LookingAt(".*\\w.*//")
                and not reader.LookingAt(" *namespace ")
                and not reader.LookingAt(" *class +\\w+ *;")
                and not reader.LookingAt(" *#")):
        ...
        if is_public_line:
            ...
            out.write(
                " " * public_line_indent + "/// @name General Members\n\n")
            out.offset += 2
